...
setversioning() {
	# For staging branch
	KERNELNAME="tokui-$LINUXVER-$DEVICE-$(TZ=Asia/Kolkata date +"%Y-%m-%d-%s")"
...
build_kernel() {
	...
	if [ "$PTTG" = 1 ]
 	then
            tg_post_msg "<b>🔨 Tokui Kernel Build Triggered</b>
